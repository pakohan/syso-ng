...
{
    if (alloc_chrdev_region(&first, 0, 1, "hello.c") < 0)
        goto error_exit;
...
static int driver_open( struct inode *device, struct file *instance )
{
    ...
    */
    instance->private_data = kmalloc(sizeof(sssize_t),GFP_KERNEL);
    ((sssize_t*) (instance->private_data))->already_copied = 0;
...
static ssize_t driver_read( struct file *instance, char *user, size_t count, loff_t *offset )
{
    ...
    to_copy = strlen(hello_world) + 1 - ((sssize_t*)instance->private_data)->already_copied;
    to_copy = min( to_copy, count );
    if (to_copy > 0)
        not_copied = copy_to_user( user, &hello_world[((sssize_t*)instance->private_data)->already_copied], to_copy);
    else
        return 0;
    ((sssize_t*)instance->private_data)->already_copied = to_copy - not_copied;
